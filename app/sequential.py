...
class LogicalPlanner:
    ...
    async def _exhaust_agent(self, agent, operation, phase):
        while True:
            ...
            if not await self.choose_next_link(operation, agent, phase):
                break
    ...
    async def choose_next_link(self, operation, agent, phase):
        ...
        for a in phase_abilities:
            ...
            if encoded_test not in completed_tests and not variables:
                links = True
                await self.data_svc.dao.create('core_chain',
                                               dict(op_id=operation['id'], host_id=agent['id'], ability=a['id'],
                                                    decide=datetime.now(), score=0, command=encoded_test,
                                                    cleanup=a.get('cleanup') if operation['cleanup'] else '',
