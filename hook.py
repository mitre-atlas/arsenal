...
async def enable(services):
    ...
    await stockpile_svc.contact_svc.register(HTTP(services), mode=c2_configs['HTTP']['mode'],
                                             enabled=c2_configs['HTTP']['enabled'])
    await stockpile_svc.contact_svc.register(GIST(services, c2_configs['GIST']), mode=c2_configs['GIST']['mode'],
                                             enabled=c2_configs['GIST']['enabled'])
